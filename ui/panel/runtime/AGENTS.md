...
- [presentation.js](presentation.js)：包含宿主明暗与语义色的只读窗口投影（缺失时恢复默认色）、Web 三材质及 liquidVariant 偏好同步（忽略弹出收起偏好），通过工作台纯模型迁移旧比例并接收两端独立布局；按钮与表情双击共用 togglePanelWindow 弹出/收回与动作校验，未收到后台 popoutSupported=true 时禁用弹出；执行业务动作前重新核对宿主；不接收远端 theme 命令，按投影设置浮窗主题和原生外观通知，不改变系统设置；内嵌弹出/收回发出 appearance 通知以同步 SVG 液态；弹出与收回使用方向相反的图标，提示对应动作；窗口切换由公共表情手势触发，旧图标隐藏；桌面头部保留置顶按钮，默认开启且可保存取消选择，写入期间禁用按钮，使用请求时的目标值避免投影回读后二次翻转；Web 设置保留同一偏好，不插入弹出专用来源行。
