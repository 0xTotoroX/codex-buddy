...
- [surfaces.js](surfaces.js)：识别前景对话框及已验证的独立聊天容器，供停靠与输入目标校验共用。
- [dock.js](dock.js)：在主内容或前景聊天中创建可撤销占位，挂载自有界面并约束层级；局部属性样式让正文/输入框共同缩窄，退出恢复，无法适配的前景界面暂时让位；几何通知包含按实际可展开空间计算的回程锚点，独立于已保存的展开意图。
