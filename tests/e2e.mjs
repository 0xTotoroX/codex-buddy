...
const output = join(root, 'target/reports/e2e');
...
try {
    ...
    desktop.evaluate(() => {
      ...
      return p
        ? {
            ...
            open: p.state.open,
            transitioning: !!p.state.morphAnimation || !!p.state.viewTransitioning,
  ...
  await waitFor(
    async () => (await api('state')).body.desktop.headings === (await panelState()).outline.length,
  ...
  const requestTrace = await desktop.evaluate(
    () =>
      window.__companionFloatingPanel.diagnostics().find((item) => item.event === 'request:started')
        ?.details,
  );
  ...
  const fontBefore = await desktop.evaluate(() => window.__companionFloatingPanel.state.fontOffset);
  ...
  assert.equal(await desktop.evaluate(() => window.__companionFloatingPanel.state.labelOnly), true);
  ...
  await desktop.mouse.up();
  await delay(200);
  const resizedBox = await desktop.locator('.csw-panel').boundingBox();
  ...
  await waitFor(
    async () =>
      (await desktop.evaluate(() => window.__companionFloatingPanel.panelPreferences())).labelOnly,
  ...
  await waitFor(async () => (await panelState())?.active, 'Reload did not restore floating panel');
  assert.equal(await desktop.locator('[data-companion-stepwise-root]').count(), 1);
  ...
  assert.equal(await desktop.locator('[data-companion-stepwise-root]').count(), 0);
  assert.equal(await desktop.evaluate(() => typeof window.__companionDesktopRequest), 'undefined');
