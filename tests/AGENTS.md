...
- [dev-launcher.test.mjs](dev-launcher.test.mjs)：开发入口重复唤起、原目标重连、失败边界、失效锁保留及 shell 转义；设置 CODEX_BUDDY_NATIVE_LAUNCHER_TEST=1 显式验收合成原生窗口的 Dock 策略与焦点交接，不在普通回归中抢焦点。
