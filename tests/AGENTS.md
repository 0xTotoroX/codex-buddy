...
模型控制条使用独立合成宿主与本机 API 验收；不读取真实聊天、不调用模型，也不修改官方应用配置。宿主适配和网页行为纳入 verify，原生窗口单独检查焦点与鼠标。纯几何回归覆盖左右/顶部端点的悬停保留区、8pt退出容错、冻结区与物理刘海排除；动画回归比较相同总时间的规则/掉帧进度。页面回归确认几何帧不重写未变配色，固定原生视口背景无 CSS 轮廓追赶；原生测试扣住首帧确认与几何事件，核验旧确认拒绝、显示门控与遮罩独立收放。
