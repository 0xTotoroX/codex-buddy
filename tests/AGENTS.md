...
- [workbench-binding.mjs](workbench-binding.mjs)：由 workbench.mjs 执行的关联回归，覆盖手动/自动 A-B-A 建议缓存恢复、重新挂载后安全填入、回答/生成设置失效，以及同目标保留、换目标迟到结果、兄弟输入框隔离、身份替换/失联/重挂载、窗口命令及冷初始化；只使用合成响应。
