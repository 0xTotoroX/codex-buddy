...
也可执行一次 `npm run install:dev`，安装独立的 **CodexBuddy Dev.app**。以后双击它会打开开发终端，保存源码自动更新；在该终端按 Ctrl+C 退出。重复打开会唤起已有工作台，断连时尝试重连原窗口，不会启动第二套开发后台或重启 ChatGPT。启动期间显示 Dock 图标，连接完成后入口退出，开发后台继续运行。它依赖本机源码和开发工具，修改开发脚本或依赖后需重开开发终端；移动源码目录或更换 Node 路径后需重新生成入口。
