...
- [model.rs](model.rs)：模型适配层，统一 CLI 与 API 请求和结构化结果、完整/限长输入及整体推进优先的中文建议；Model、ModelInfo、Suggestion 与生成/测试/模型查询。
...
- [settings.rs](settings.rs)：分开维护保存版本与生成版本，大纲切换不取消生成；外部读取只返回密钥配置状态；Options、Update 及设置读取、校验和保存（maxInputChars 默认 0 表示完整最近一问一答，兼容旧正数上限）；返回后台检测的只读 popoutSupported。
