...
- [dev.mjs](dev.mjs)：真实 Codex 开发入口，管理独立后台、源码监听和编译失败回退；启动前先发现可调试宿主，仅在接管安装版后启用退出恢复，失败原因写入私有 launcher-error.json；Ctrl+C 清理自身进程并恢复安装版连接。
...
- [dev-host.mjs](dev-host.mjs)：选择真实窗口，首次复制独立开发配置，暂停并恢复安装版连接；新目标已发现且旧目标不可用时将旧恢复记录保留为带唯一后缀的归档，不将安装版改连新聊天；API 错误保留后端原因；不创建浏览器或修改官方应用。
