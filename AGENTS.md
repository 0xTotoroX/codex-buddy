...
- 工作台控制图标统一使用 OpenAI 公开 apps-sdk-ui 的 MIT SVG 子集，来源和许可位于 `ui/panel/icons/`，分发自动收集署名；统一 18px 图形和至少 28px 点击区域，不混用文本符号或各自缩放。公共表情保持顶部中轴与胶囊相同的 84×36 尺寸；鼠标操作无焦点外圈，键盘聚焦使用中性色细线；来源与操作各占等宽区域，不能挤偏表情。
- 工作台和胶囊前端以简洁、直观的高频操作为主；新增可见按钮或菜单须慎重评估，完整功能与低频设置优先集中到 Web 设置页，不因为新增能力就默认增加前端入口。聊天来源文字、关联及编排下拉暂时隐藏，保留已有状态与功能逻辑。鼠标进入整个工作台时统一显现顶部操作和面板刷新，离开工作台后统一隐藏；键盘聚焦仍显现对应操作，触屏保持可见，透明度过渡尊重减少动态效果；面板标题旁只保留刷新；标题/标签拖动编排、双击临时放大，Esc 恢复；头部使用明确的移到独立窗口/放回聊天、内嵌收起与设置动作；齿轮直接打开外部 Web 设置，旧内置设置页及处理器保留但入口始终隐藏，位置、排列、置顶和明暗集中到 Web 设置。
