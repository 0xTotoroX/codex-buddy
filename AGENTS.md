...
- `configurationRevision` 处理并发设置保存；`generationRevision` 仅在有效生成输入变化时递增。大纲开关不使建议或生成中请求失效。窗口的上下文、建议和大纲分别校验身份。Stepwise 默认使用完整最近一问一答（maxInputChars=0），前端采集、CDP 生成请求与模型适配均不做本地字符截断；已有正数上限保留，Web 设置可切换完整或自定义上限。建议围绕整体目标，优先承接依据与参考线索、追问真实遗漏或假设、解释必要术语；每条可独立发送，按实际上下文调整，不硬凑角色或机械拆分回答。一次请求返回多条中文建议，保留共同待办、澄清确认与互斥方案边界。quickPrompts 默认「继续」「执行」，Web 可自定义，点击仅填入并保护草稿；修改快捷词不改变 generationRevision 或清空建议缓存。
