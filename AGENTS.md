...
- 工作台和胶囊前端以简洁、直观的高频操作为主；新增可见按钮或菜单须慎重评估，完整功能与低频设置优先集中到 Web 设置页，不因为新增能力就默认增加前端入口。聊天来源文字、关联及编排下拉暂时隐藏，保留已有状态与功能逻辑。顶部操作和面板刷新在各自标题区域悬停或键盘聚焦时显现，触屏保持可见，透明度过渡尊重减少动态效果；面板标题旁只保留刷新；标题/标签拖动编排、双击临时放大，Esc 恢复；头部使用明确的移到独立窗口/放回聊天、内嵌收起与设置动作，位置、排列、置顶和明暗集中到 Web 设置。
