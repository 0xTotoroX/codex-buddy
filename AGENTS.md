...
- 胶囊继续使用原生 CSS，styles/ 按变量、布局、内容、控件、材质、动画分层；native.css 只处理原生覆盖。Web 设置页使用 Tailwind CSS v4 与本地 shadcn/ui 基础组件；工具类及 Preflight 仅进入设置页构建。ui/tokens.css 共享语义色和通用尺寸，各界面不共享 reset。模型快切固定在所选显示器，使用 CanJoinAllSpaces/FullScreenAuxiliary 跨桌面与全屏空间显示，不依赖宿主焦点或可见性；宿主失联时保留控制条，但目标无效时禁止模型操作。后台服务断开或租约失效时退出。首次点击模型组合先回读官方配置并保留可支持的速度，未识别目标时明确提示并禁止写入。模型快切默认纯黑不透明贴边外壳，可独立选择哑光、磨砂或液态（Regular/Clear）；复用原生材质与图标、哑光、磨砂和液态只读继承宿主主题，所有材质继承字号，不改变工作台材质。设置页与控制条菜单均可选择显示器和位置。预设栏、共用表头矩阵、单行来源为默认界面，搜索与详情按需打开。悬停使用不随动画变化的入口/展开区域并保留原入口，展开退出容许8pt抖动，点击仍按精确轮廓穿透；原生鼠标进入立即展开，离开450ms收起，编辑/拖动/鼠标按住/业务操作期间保持；Esc缩窗前冻结带容错的原悬停区，实际离开才允许再次悬停展开。
