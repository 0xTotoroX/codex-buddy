...
- 工作台控制图标统一使用 OpenAI 公开 apps-sdk-ui 的 MIT SVG 子集，来源和许可位于 `ui/panel/icons/`，分发自动收集署名；统一 18px 图形和至少 28px 点击区域，不混用文本符号或各自缩放。公共表情保持顶部中轴与胶囊相同的 84×36 尺寸；展开态表情只显示眼睛，不绘制胶囊底色、边框或光晕；紧凑胶囊键盘聚焦使用中性色细线，展开表情使用眼睛下方短线；来源与操作各占等宽区域，不能挤偏表情。
...
- 弹出强调色随宿主投影同步，和内嵌共用 Codex 的实际主题色；明暗跟随 macOS，标题栏按钮经 System Events 公共脚本接口切换系统明暗，失败明确提示；不再执行宿主 theme 命令或采用投影明暗。内嵌只读跟随 Codex 明暗和强调色，不提供宿主颜色控制；Web 设置以大纲导航跳转分组，窄屏切为顶部导航。弹出所有展开视图共用可调整尺寸，原生窗口变化同步 WebView，再保存宽高；设置页也允许左右下角缩放。
