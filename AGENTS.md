...
- `configurationRevision` 处理并发设置保存；`generationRevision` 仅在有效生成输入变化时递增。大纲开关不使建议或生成中请求失效。窗口的上下文、建议和大纲分别校验身份。Stepwise 默认使用完整最近一问一答（maxInputChars=0），前端采集、CDP 生成请求与模型适配均不做本地字符截断；已有正数上限保留，Web 设置可切换完整或自定义上限。建议围绕整体目标按上下文决定内容与顺序，不固定首条或各条角色，不机械拆分回答；保留完整推进共同待办的选择及澄清、确认和互斥方案边界。
