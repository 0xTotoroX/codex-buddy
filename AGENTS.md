...
- 胶囊继续使用原生 CSS，styles/ 按变量、布局、内容、控件、材质、动画分层；native.css 只处理原生覆盖。Web 设置页使用 Tailwind CSS v4 与本地 shadcn/ui 基础组件；工具类及 Preflight 仅进入设置页构建。ui/tokens.css 共享语义色和通用尺寸，各界面不共享 reset。模型快切固定在所选显示器，使用 CanJoinAllSpaces/FullScreenAuxiliary 跨桌面与全屏空间显示，不依赖宿主焦点或可见性；宿主失联时保留控制条，但目标无效时禁止模型操作。后台服务断开或租约失效时退出。首次点击模型组合先回读官方配置并保留可支持的速度，未识别目标时明确提示并禁止写入。模型快切默认纯黑不透明贴边外壳，可独立选择哑光、磨砂或液态（Regular/Clear）；复用原生材质与图标、哑光、磨砂和液态只读继承宿主主题，所有材质继承字号，不改变工作台材质。设置页与控制条菜单均可选择显示器和位置。预设栏、共用表头矩阵、单行来源为默认界面，搜索与详情按需打开。模型快切收放保持 NSPanel/WebView/原生背景视口稳定，由根视图遮罩统一裁切，网页内容预先布局；首帧与键盘交接等待对应场景就绪，迟到确认不能重新展开。悬停使用不随动画变化的入口/展开区域并保留原入口，展开退出容许8pt抖动，点击仍按精确轮廓穿透；原生鼠标进入立即展开，离开450ms收起，编辑/拖动/鼠标按住/业务操作期间保持；Esc缩窗前冻结带容错的原悬停区，实际离开才允许再次悬停展开。
...
- 多 worktree 调试由一个 Dev 监督进程交接来源；`npm run dev:settings` 打开稳定的开发设置地址，显示路径、分支、提交、未提交状态和实际资源回读。选择器由监督进程注入设置页，旧源码分支不必先合并管理器。先编译再交接，失败恢复原来源；同一宿主目标使用跨 worktree 的独占记录，旧独立 Dev 会话需正常退出后再用统一入口。每个来源保留独立配置和构建目录，不迁移运行期建议缓存。设置地址与令牌在切换期间保持，旧标签页写入通过来源标识拒绝；切换不重启 Codex、不合并代码。来源选择存于 Git 公共目录，源码归档仅提供当前目录；`--source WORKTREE_PATH` 可显式覆盖启动来源。开发脚本升级仍需一次正常重开 Buddy，日常来源切换不需重新安装。
...
- 胶囊、内嵌工作台、桌面浮窗和模型快切的哑光、磨砂、液态在所有交互/展开状态下只读跟随 Codex 明暗与语义颜色；浮窗投影携带背景、文字、弱化文字、强调、悬停、分隔和错误颜色，原生窗口按宿主主题设置自身 NSAppearance，不修改系统或 Codex。模型快切 black 材质在所有状态保持固定纯黑配色和深色原生外观；断线或无有效主题时保留最后一次主题。Web 设置页及大纲继续跟随浏览器系统主题，以大纲跳转分组，窄屏切为顶部导航。弹出所有展开视图共用可调整尺寸，原生窗口变化同步 WebView，再保存宽高；设置页也允许左右下角缩放。
- 桌面浮窗没有固定最大宽高，只保留最小可用尺寸；大尺寸偏好在内嵌交接时保留，内嵌实际渲染仍受胶囊与宿主空间约束。弹出窗口只接收建议、大纲和状态投影；宿主保留任务识别与写入校验。内容 ready 与窗口 presented 分开确认，原生窗口在来源位置呈现后才淡出内嵌，随后完成提起移动；收回先移动至来源，再确认内嵌恢复并关闭；宿主恢复失败保留浮窗。交接时按任务和内容身份恢复阅读位置，普通投影不覆盖用户阅读；这些临时数据及屏幕交接坐标不写入 `panel.json`。有效屏幕坐标下入场 320ms、收回移动 260ms，以整窗位置/尺寸和共同容器轻缩放接续；屏幕断开或坐标无效时短过渡，减少动态效果时立即完成。开发版额外试验 WindowServer 私有网格形变：固定窗口内容尺寸，仅改变合成后的网格，朝目的地曲面收束，弹出沿收回路径反向展开，首帧显示前预置来源网格；起终点使用 NSScreen 全局逻辑坐标，可跨主屏与随航，宿主页面不可见时仍从布局读取返回区域；正式构建禁用，符号缺失或接口报错时回退原有空间动画。移动中不持久保存中间位置/尺寸，拖动可接管、收回途中可双击取消；结束眨眼一次。弹出只保留展开态，单击眼睛、Esc 和旧收起偏好均不能折叠，双击眼睛收回 Codex 并恢复出发时的胶囊/展开态。内嵌紧凑表情单击展开、双击直接弹出（无需先展开）；展开后的公共表情单击收起、双击弹出，头部不再显示收起按钮；returnOpen 独立保存出发形态，100ms 首击已执行时仍以首击前形态为准，弹出过程不覆盖它；支持弹出时鼠标单击等待 100ms，键盘立即响应；第二击由稳定外层识别，可取消尚未执行的单击或中断已开始的收放并切换窗口；窗口切换图标隐藏。内嵌手动缩放固定对角、边缘与指针 1:1 移动，展开动画仍围绕胶囊。窗口与内嵌动效均尊重减少动态效果设置。
- 停靠工作台为显式可选布局，旧配置保留胶囊。在已识别的主内容或独立前景聊天内创建占位，将自有根节点挂在占位内；不移动宿主节点，前景聊天通过可撤销的专属属性样式让正文和输入框共同让位。设置或未知前景界面出现时暂时隐藏，关闭后恢复，临时让位不覆盖保存的开合偏好。大纲与 Stepwise 同时显示、独立刷新滚动、使用同一上下文；按聊天实际可分配空间临时缩窄侧栏，不覆盖保存宽度；主动收起或空间不足时归为共用胶囊并释放全部侧栏占位，保留停靠位置偏好；胶囊单击恢复侧栏，放不下时提供原因及独立窗口/聊天内展开，不自动换位置或重新展开。工作台支持自动/上下/左右排列、交换位置与恢复默认，以及标题/标签拖拽、合并/拆分标签和临时专注；完整编排选项集中在 Web 设置；默认双面板，自动布局不拆标签，专注不持久化，拖动只预览且不移动原生浮窗；停靠和浮窗各自保存排列、首个面板及双轴比例，空间不足只改变实际布局，自动切换有滞回。停靠宽度与分栏比例独立于浮窗宽高，临时阅读状态按聊天和各自内容身份校验，仍复用现有一个原生窗口。
...
- `configurationRevision` 处理并发设置保存；`generationRevision` 仅在有效生成输入变化时递增。大纲开关不使建议或生成中请求失效。窗口的上下文、建议和大纲分别校验身份。Stepwise 默认使用完整最近一问一答（maxInputChars=0），前端采集、CDP 生成请求与模型适配均不做本地字符截断；已有正数上限保留，Web 设置可切换完整或自定义上限。方向来源 directionSource 独立于生成时机 generationMode：自动探索自由发现方向，自选使用有序方向位置，智能挑选通过可关闭的 Jev 批量判断筛选候选后共用生成。六个预设方向及自定义名称/说明在 Web 编辑；自选最多按位置数生成，其余按 maxItems 上限，允许少给或为空。新配置默认最多三条，已有配置保留数量。问题与回答以 Exchange 分开传递，正数预算优先保留用户问题并标记截断；Jev 与生成共用快照、取消与总超时。Jev 默认未同意，不复用生成密钥，仅显式启用并同意后发送；失败不降级，零合适方向不生成。建议保持整体目标与授权边界、互补独立，不固定承接/追问/解释角色。quickPrompts 默认「继续」「执行」，Web 可自定义，点击仅填入并保护草稿；修改快捷词不改变 generationRevision 或清空建议缓存。
...
- Web 设置模型表单使用失焦/选项切换自动保存，保留手动保存；共用串行队列及 configurationRevision，保留在途新编辑、失败草稿和外部冲突提示。Web 设置覆盖胶囊可配置项，panel 仅保留直接打开网页的齿轮入口；外观偏好逐项保存并双向同步；开发内嵌和弹出在“外观”选项框内的液态旁独立星星切换 Clear，点亮为 Clear；Web 设置同步该偏好，安装版内嵌回退时隐藏星星。Web 保留现有布局、图标和视觉样式；用留白代替重复说明，只保留影响操作判断的小字提示。
- README.md 默认英文，README.zh-CN.md 提供中文并保持内容同步；其余文档默认中文；代码标识符英文。结构变更同步公开 README、模块地图及 Obsidian 内的相关维护记录；不建立插件框架或额外桌面管理器。
- 长期项目决策先查本地 `buddy.docsPath` 所指资料目录的 `AGENTS.md` 与关联笔记；其中只记录稳定约定和导航，详细讨论、验收及审查分别成文。可复用操作步骤进入已有 Runbook；私人资料不参与构建或分发。

## 版本与发布约定

- 版本统一使用 `X.Y.Z`，当前为 `0.8.0（早期开发阶段）`。`0.x.x` 表示尚在早期开发，不代表完成比例。
...
模型控制条独立于大纲/下一步工作台：设置页或 `codex-buddy model-control` 开启，默认右侧贴边，支持左侧与顶部物理刘海/普通顶部回退；首次启用后保存启动偏好。只控制已连接且能唯一识别的输入目标，工作台阅读锁定不改变它。模型能力来自关联的官方 model/list，与菜单可见选项匹配；未知时等待，不硬编码模型或自动重启宿主。菜单操作串行执行并回读；生成状态不单独阻塞切换，遵守官方控件禁用状态，不排队；部分失败显示实际状态，不宣称原子回滚。预设、屏幕/边缘位置与独立材质保存在 model-control.json，后台退出时控制窗随租约结束。复用原有图标，模型控制不进入建议生成 model.rs。
...
- `README.md` / `README.zh-CN.md`：默认英文与中文版，互设语言入口；只说明功能、安装使用、更新卸载、二次开发入口及必要的隐私/兼容限制；材质实现、内部机制、详细验收和维护过程留在 Obsidian。全量文档同步不扩大公开 README。
...
展开外壳统一使用工作台，胶囊仅为紧凑形态；layoutMode 保留原位置偏好，不再选择另一套展开外壳。公共表情使用与胶囊相同模板/状态，在分栏、标签、专注和设置中常驻；内嵌表情单击收放、双击弹出；桌面表情仅双击收回，内嵌头部不再显示收起和窗口往返按钮；只用表情收放与双击往返。浮窗自身的位置/置顶/外观写入串行并及时更新修订号，保留真正外部并发冲突保护。
