...
- 胶囊继续使用原生 CSS，styles/ 按变量、布局、内容、控件、材质、动画分层；native.css 只处理原生覆盖。Web 设置页使用 Tailwind CSS v4 与本地 shadcn/ui 基础组件；工具类及 Preflight 仅进入设置页构建。ui/tokens.css 共享语义色和通用尺寸，各界面不共享 reset。模型快切仅随已连接宿主获得焦点时进入其桌面，不加入所有Spaces；宿主不可见或失联时隐藏，后台悬停/快捷键不搬动桌面。首次点击模型组合先回读官方配置并保留可支持的速度，未识别目标时明确提示并禁止写入。模型快切采用独立不透明纯黑贴边外壳，只读继承字号与共享图标；工作台三种材质保持原样。预设栏、共用表头矩阵、单行来源为默认界面，搜索与详情按需打开。原生鼠标进入立即展开，离开450ms收起，编辑/拖动/鼠标按住/业务操作期间保持；Esc缩窗前冻结原命中区，实际离开才允许再次悬停展开。
