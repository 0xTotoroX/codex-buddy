...
- 工作台和胶囊前端以简洁、直观的高频操作为主；新增可见按钮或菜单须慎重评估，完整功能与低频设置优先集中到 Web 设置页，不因为新增能力就默认增加前端入口。聊天关联及编排下拉暂时隐藏，保留来源文字、已有状态与功能逻辑。面板标题旁只保留刷新；标题/标签拖动编排、双击临时放大，Esc 恢复；头部使用明确的移到独立窗口/放回聊天、内嵌收起与设置动作，位置、排列、置顶和明暗集中到 Web 设置。
...
- 停靠工作台为显式可选布局，旧配置保留胶囊。在已识别的主内容或独立前景聊天内创建占位，将自有根节点挂在占位内；不移动宿主节点，前景聊天通过可撤销的专属属性样式让正文和输入框共同让位。设置或未知前景界面出现时暂时隐藏，关闭后恢复，临时让位不覆盖保存的开合偏好。大纲与 Stepwise 同时显示、独立刷新滚动、使用同一上下文；空间不足收为窄入口，不自动弹出或自动重新展开。工作台支持自动/上下/左右排列、交换位置与恢复默认，以及标题/标签拖拽、合并/拆分标签和临时专注；完整编排选项集中在 Web 设置；默认双面板，自动布局不拆标签，专注不持久化，拖动只预览且不移动原生浮窗；停靠和浮窗各自保存排列、首个面板及双轴比例，空间不足只改变实际布局，自动切换有滞回。停靠宽度与分栏比例独立于浮窗宽高，临时阅读状态按聊天和各自内容身份校验，仍复用现有一个原生窗口。
