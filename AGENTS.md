...
- 停靠工作台为显式可选布局，旧配置保留胶囊。在已识别的主内容或独立前景聊天内创建占位，将自有根节点挂在占位内；不移动宿主节点，前景聊天通过可撤销的专属属性样式让正文和输入框共同让位。设置或未知前景界面出现时暂时隐藏，关闭后恢复，临时让位不覆盖保存的开合偏好。大纲与 Stepwise 同时显示、独立刷新滚动、使用同一上下文；空间不足收为窄入口，不自动弹出或自动重新展开。停靠宽度与分栏比例独立于浮窗宽高，临时阅读状态按聊天和各自内容身份校验，仍复用现有一个原生窗口。
