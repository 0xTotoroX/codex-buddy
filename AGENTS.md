...
- `npm run install:dev` 生成独立 CodexBuddy Dev.app，绑定当前源码与 Node 路径，双击打开终端运行现有开发流程；重复启动唤起现有工作台，断连只尝试重连原目标；启动期间显示 Dock 图标，AppKit 协作交接焦点后退出入口。Ctrl+C 退出开发，不更新日常安装。移动源码或 Node 后重新生成入口。
