...
在 Web 设置中选择 **输入上下文 → 最近一次聊天（完整）**，即可使用完整的最近一次提问与回答，不做本地字符截断。新配置默认采用此模式；已有字符上限保持原值，也可继续自定义。实际输入仍受所选模型上下文窗口限制。一次请求返回多条可独立发送的中文建议，优先承接回答与参考线索、追问遗漏或假设、解释必要术语，按实际内容调整。共同待办可以整体推进，不机械拆成单项选择。
