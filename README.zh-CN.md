...
在 Web 设置中选择 **生成上下文 → 最近一次聊天（完整）**，即可使用完整的最近一次提问与回答，不做本地字符截断。新配置默认采用此模式；已有字符上限保持原值，也可继续自定义。实际输入仍受所选模型上下文窗口限制。回答包含同一目标下的多项待办时，建议优先提供一条完整推进方案，其余建议补充检查或替代思路。建议使用中文生成。
