...
大纲与下一步始终使用同一聊天来源，顶部保留来源文字。跟随／锁定菜单暂时隐藏，已有的关联状态保留。来源暂不可用时保留已有结果供查看，恢复后继续使用。
...
- **同时查看**：在 Web 设置的「聊天内位置」选择「固定在聊天右侧」或「在聊天内自由移动」。拖动左边界调宽、内部分隔线调整比例；拖动标题调整排列，Web 设置中可选自动、上下、左右、顺序或恢复默认；单独打开聊天时跟随到聊天内，关闭后回到主界面；空间不足时收起，点击右侧入口重新打开。整个工作台仍可弹出或收回。
